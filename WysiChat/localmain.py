...
# --- DRAG & DROP SETUP (tkinterdnd2) ---
try:
    import tkinter as tk
    import tkinterdnd2
    from tkinterdnd2 import DND_FILES, TkinterDnD
    
    # Explicitly set TKDND_LIBRARY path to help Tcl find the binaries
    _dnd_path = os.path.join(os.path.dirname(tkinterdnd2.__file__), 'tkdnd')
    _dnd_pkg_dir = os.path.dirname(tkinterdnd2.__file__)
    _dnd_path = os.path.join(_dnd_pkg_dir, 'tkdnd')
    
    # If direct path doesn't exist, try to find any folder starting with tkdnd (e.g. tkdnd2.9)
    if not os.path.exists(_dnd_path):
        for item in os.listdir(_dnd_pkg_dir):
            if item.startswith('tkdnd') and os.path.isdir(os.path.join(_dnd_pkg_dir, item)):
                _dnd_path = os.path.join(_dnd_pkg_dir, item)
                break

    if os.path.exists(_dnd_path):
        os.environ['TKDND_LIBRARY'] = _dnd_path

    # Save original Tk to prevent recursion in TkinterDnD
    _original_Tk = tk.Tk
    
    class DnDTkWrapper(TkinterDnD.Tk):
        def __init__(self, *args, **kwargs):
            global DND_ENABLED
            # Temporarily restore original Tk so TkinterDnD.Tk.__init__ calls the real Tk
            tk.Tk = _original_Tk
            try:
                ...
                # Re-apply patch only if DnD is still enabled
                if DND_ENABLED:
                    tk.Tk = DnDTkWrapper

    # Monkeypatch Tk so FreeSimpleGUI uses the DnD-enabled root
    tk.Tk = DnDTkWrapper
    DND_ENABLED = True
except ImportError:
    DND_ENABLED = False
...
# --- LOGGING SETUP ---
log_file = os.path.join(script_dir, "fyr_chat_debug.log")
logging.basicConfig(
    filename=log_file, 
    level=logging.DEBUG, 
...
try:
    ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID('FYR.Chat.App.1.0')
...
# --- SHARED_PATH = r"C:\Supergirl\Tools\FYRTools\Wysichat"
SHARED_PATH = r"C:\Supergirl\Tools\FYRTools\Wysichat"
...
def main():
    ...
    window_icon = resource_path("wysiwyglogo.png") # Use PNG for Window to prevent TclError
    tray_icon = resource_path("wysiwyglogo.png")   # Use PNG for Tray as well to prevent TclError
    alert_icon = resource_path("fyrlogo_alert.png") # Icon to show when message arrives
    create_alert_icon(alert_icon)
    
    win_loc = sg.user_settings_get_entry('-WIN-LOC-', None)
    
    window = sg.Window("WysiChat", layout, finalize=True, icon=window_icon, resizable=True, use_custom_titlebar=use_custom_title, enable_close_attempted_event=True, location=win_loc)
    ...
    #         window.TKroot.dnd_bind('<<Drop>>', lambda e: window.write_event_value('-DROP-', e.data))
    #     except Exception as e:
    #         logging.error(f"DnD Setup Failed: {e}")
    ...
    def create_tray_window():
        ...
        menu = ['', ['Restore', 'Run FYRlister', 'Exit']]
        
        # Use Magenta as transparency key to hide the window background
        bg_col = '#ff00ff'
        layout = [[sg.Image(filename=tray_icon, key='-TRAY-IMG-', background_color=bg_col, right_click_menu=menu, tooltip='WysiChat')]]
    ...
    def save_loc():
        try:
            ...
            if loc and loc != (None, None):
                sg.user_settings_set_entry('-WIN-LOC-', loc)
            if window.TKroot:
                ...
                if len(parts) >= 4:
                    ...
                    # Only save if coordinates are realistic (avoid -32000 minimized coords)
    ...
    while True:
        ...
        if tray_event in ('Restore', '-TRAY-IMG-_DOUBLE'):
            ...
            tray_flashing = False
            try:
                tray_window['-TRAY-IMG-'].update(filename=tray_icon)
            except Exception:
                pass
            unread_count = 0
        ...
        if event == sg.TIMEOUT_EVENT:
            ...
            if tray_flashing:
                ...
                if flash_timer >= 500: # Toggle every 500ms
                    ...
                    new_icon = alert_icon if flash_state and os.path.exists(alert_icon) else tray_icon
                    try:
                        tray_window['-TRAY-IMG-'].update(filename=new_icon)
                    except Exception:
